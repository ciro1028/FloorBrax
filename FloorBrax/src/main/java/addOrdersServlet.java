...
@WebServlet(name = "addOrdersServlet", urlPatterns = {"/addOrdersServlet"})
public class addOrdersServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            String formattedDate = dateArray[2] + "/";
            ...
            order.insertOrder(orderNum, installerId, description, formattedDate, status, subdivisionId, pic, total, toPay, serviceList, sqf, userId, extraPrice, sqfDitra, sqfHardwood);
            user.selectUser("id", order.getUser());
            order.selectOrder("order_num", orderNum);            
            
            session.setAttribute("order", order);
            session.setAttribute("sub", subdivision.getName());
            session.setAttribute("inst", installer.getName());
