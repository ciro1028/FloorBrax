...
<body>
	...
	<div class="container">
		<form:form action="getInstaller" method="post" modelAttribute="installer">
			<div class="input-group mt-2">
				<form:input path="name" class="form-control" type="text" name="name" placeholder="Enter Name of Installer" required="true" />
				...
				<table class="table table-striped">
					...
					<tbody>
						<tr>
							...
							<td class="text-center">${inst.id}</td>
							<td class="text-center">${inst.name}</td>
							<td class="text-center">${inst.phone}</td>
							<td class="text-center">
								<c:url var="viewInstallerLink" value="/installer/viewInstaller">
									<c:param name="installerId" value="${inst.id}"></c:param>
								</c:url> <a class="btn btn-primary" href="${viewInstallerLink}">View</a>
							</td>
						</tr>
					</tbody>
				</table>
	...
	<div class="container">
		<div class="row">
			<button class="btn btn-link" type="submit" onclick="showAll()">Show all installers</button>
		</div>
	</div>
			...
			<table class="table table-striped">
				<thead>
					<tr>
						...
						<th>Name</th>
						<th>Phone</th>
						<th>View Installer</th>
					</tr>
				</thead>
				<tbody>
					<c:forEach items="${installers}" var="installer" varStatus="loop">
						<tr>
							<th scope="row">${loop.index}</th>
							<td><c:out value="${installer.id}" /></td>
